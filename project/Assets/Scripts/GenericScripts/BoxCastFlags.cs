...
public class BoxCastFlags : MonoBehaviour
{
	...
	void Update()
	{
		...
		//Raycast
		isStay = Physics.BoxCast(position + myTransform.right * m_center.x + myTransform.up * m_center.y + myTransform.forward * m_center.z,
			boxCastScale, GetBoxCastDirection(), out raycastHit, rotation, m_distance, m_raycastLayerMask);
	...
	void OnDrawGizmos()
	{
		...
		//呼び出しコスト削減
		Transform myTransform = transform;
		Vector3 direction = GetBoxCastDirection();
		Vector3 center = myTransform.right * m_center.x + myTransform.up * m_center.y + myTransform.forward * m_center.z;
		//Hit
		if (isStay)
		{
			//Rayの距離を求める
			...
			//Draw Ray
			Gizmos.DrawRay(myTransform.position + center, direction * distance);
			//Matrix
			Gizmos.matrix = Matrix4x4.Translate(myTransform.position + center + direction * distance);
		...
		else
		{
			...
			//Draw Ray
			Gizmos.DrawRay(myTransform.position + center, direction * m_distance);
			//Matrix
			Gizmos.matrix = Matrix4x4.Translate(myTransform.position + center + direction * m_distance);
			Gizmos.matrix *= Matrix4x4.Rotate(myTransform.rotation);
			//Draw Cube
			Gizmos.DrawWireCube(Vector3.zero, Vector3.Scale(myTransform.lossyScale, m_scale));
		}
